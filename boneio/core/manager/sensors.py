...
class SensorManager:
        ...
        try:
            ...
            try:
                ...
                for sensor in sensors:
                    ...
                    _addr = sensor.id
                    _LOGGER.debug("Found Dallas GPIO device on bus %s with address %s", bus_id, _addr)
                    out[_addr] = bus_id
    ...
    async def _broadcast_adc_states(self) -> None:
        ...
        for sensor in self._adc_sensors:
            try:
                self._manager.event_bus.trigger_event(
                    SensorEvent(
                        entity_id=sensor.id,
                        state=SensorState(
                            id=sensor.id,
    ...
    def configure_virtual_energy_sensors(self) -> None:
        ...
        for config in self._virtual_energy_sensor_configs:
            ...
            if not output:
                _LOGGER.error("Output '%s' not found for virtual_energy_sensor '%s'", output_id, name)
                continue
            ...
            if sensor_id in used_ids:
                _LOGGER.error("Duplicate virtual_energy_sensor ID '%s' (from name '%s'). Skipping.", sensor_id, name)
                continue
            ...
            if sensor_type == "power" and power_usage is None:
                _LOGGER.error("power_usage required for sensor_type='power' in virtual_energy_sensor '%s'", name)
                continue
            if sensor_type == "water" and flow_rate is None:
                _LOGGER.error("flow_rate required for sensor_type='water' in virtual_energy_sensor '%s'", name)
                continue
        ...
        if self._virtual_energy_sensors:
            _LOGGER.info("Configured %d virtual energy sensors", len(self._virtual_energy_sensors))
    ...
    async def reload_virtual_energy_sensors(self) -> None:
        ...
        _LOGGER.debug(
            "Virtual energy sensors - current: %s, new: %s, duplicates: %s", current_ids, new_ids, duplicate_ids
        )
                ...
                for sensor in self._virtual_energy_sensors:
                    if sensor.id == sensor_id:
                        ...
                        if new_name and sensor.name != new_name:
                            _LOGGER.debug(
                                "Updating virtual energy sensor %s name: %s -> %s", sensor_id, sensor.name, new_name
                            )
                            sensor._name = new_name
    ...
    def _publish_virtual_energy_discovery(
        ...
        Args:
            ...
            area: Optional area/room. Special value '_same_as_output_' uses
                the linked output's area and device block.
            output_id: Linked output ID (required when area='_same_as_output_').
        """
        from boneio.integration.homeassistant import ha_virtual_energy_sensor_availabilty_message

        cfg = self._manager._config_helper

        # Resolve "_same_as_output_" — use the output's area and device block
        output_discovery = None
        effective_area = area
        if area == "_same_as_output_" and output_id:
            output = self._manager.outputs.get_output(output_id)
            if output:
                output_area = getattr(output, "area", None)
                effective_area = output_area
                # Generate the output's HA discovery payload to extract device block
                ...
                ha_func = _ha_funcs.get(output.output_type, ha_switch_availabilty_message)
                output_discovery = ha_func(
                    id=output_id,
                ...
                _LOGGER.debug(
                    "VES %s: using output %s device block (area=%s)",
                    sensor_id,
                    output_id,
                    output_area,
                )
                ...
                _LOGGER.warning(
                    "VES %s: output %s not found for _same_as_output_, falling back to no area",
                    sensor_id,
                    output_id,
                )
                effective_area = None
        ...
        for _id, _name, unit, dev_cls, state_cls in pairs:
            payload = ha_virtual_energy_sensor_availabilty_message(
                ...
                area=effective_area,
                output_discovery_payload=output_discovery,
            )
