...
async def get_parsed_config():
    ...
    try:
        ...
        # Let extension modules (e.g. expander) add derived fields — such as the
        # transient `boneio_output` alias that surfaces MCP/expander outputs in
        # the Output Groups member picker. Mutates config_data in place; the same
        # object is cached below, so cache hits stay enriched too. Counterpart:
        # ModuleRegistry.strip_for_save() in update_section_content().
...
async def update_section_content(section: str, data: dict | list = Body(...)):
    ...
    # Let extension modules strip any transient fields they injected in
    # enrich_config_response (e.g. expander's `boneio_output` alias) so the
    # YAML written to disk stays clean — round-trip GET→edit→PUT is lossless.
    from boneio.modules._registry import ModuleRegistry
